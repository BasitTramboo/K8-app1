...
def calculate():
    ...
    response = requests.post(url="http://app2-service.default.svc.cluster.local/sum",json=request.json, headers={'Content-Type': 'application/json'})
...
def store_file():
    ...
    # 2. Store the file.
    try:
        with open("/Basit_PV_dir/" + request.json["file"], "w+") as csvfile:
